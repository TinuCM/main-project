...
# Modify the button to include the image
    ...
    def show_process_manager(self):
        ...
        # Process name input
        ctk.CTkLabel(form_frame, text="Process Name (with extension):").pack()
        process_entry = ctk.CTkEntry(form_frame)
    ...
    def show_limits_manager(self):
        ...
        # CPU limit input
        ctk.CTkLabel(form_frame, text="CPU Limit (%):").pack()
        cpu_entry = ctk.CTkEntry(form_frame)
        cpu_entry.insert(0, str(self.resource_limits['cpu']))
        cpu_entry.pack(pady=(0, 10))
        
        # Memory limit input
        ctk.CTkLabel(form_frame, text="Memory Limit (%):").pack()
        memory_entry = ctk.CTkEntry(form_frame)
        memory_entry.insert(0, str(self.resource_limits['memory']))
        memory_entry.pack(pady=(0, 10))
                    ...
                    if partition.fstype and partition.device in self.disk_frames:
                        try:
                            ...
                            if label.winfo_exists() and progress.winfo_exists():
                                total_gb = usage.total / (1024**3)
                                used_gb = usage.used / (1024**3)
                                label.configure(text=f"{used_gb:.1f}/{total_gb:.1f}GB ({usage.percent:.1f}%)")
                                progress.set(usage.percent / 100)
                        except Exception:
                            continue
    ...
    def _show_alerts(self):
        ...
        if not self.alerts:
            ...
            for alert in self.alerts:
                ...
                # Priority indicator
                priority_color = "red" if alert["priority"] == "High" else "orange"
                priority_label = ctk.CTkLabel(
                    header_frame,
                    text=f"⚠ {alert['priority']}",
    ...
    def monitor_resources(self):
        ...
        try:
            ...
            # Check for high-usage processes
            for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent']):
                try:
                    ...
                    # Check if process is using high resources
                    if proc_cpu > self.resource_limits['cpu'] or proc_memory > self.resource_limits['memory']:
                        if proc_name not in self.process_whitelist:
                            # Unknown process using high resources
                            self.handle_process_alert(
                                proc_name, 
                                proc_cpu, 
                                proc_memory, 
                                "Unknown process exceeding resource limits"
                            )
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    continue

            # System-wide resource checks
    ...
    def handle_process_alert(self, process_name, cpu_usage, memory_usage, reason):
        ...
        try:
            current_time = datetime.now()
            formatted_time = current_time.strftime("%Y-%m-%d %H:%M:%S")
            alert_message = f"Suspicious process detected: {process_name}"
            ...
            alert = {
                ...
                "details": (f"Process: {process_name}\n"
                          ...
                          f"Resource Limits - CPU: {self.resource_limits['cpu']}%, Memory: {self.resource_limits['memory']}%\n"
                          f"Reason: {reason}"),
                "time": current_time.strftime("%H:%M:%S")
            }
            ...
            print(f"Error in handle_process_alert: {e}")
